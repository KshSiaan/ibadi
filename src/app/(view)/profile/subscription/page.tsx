...
export default function Page() {
  ...
  const { mutate: takeFree, isPending: freePending } = useMutation({
    ...
    mutationFn: async () => {
      const response = await fetch(
        `${base_url}${base_api}/subscriptions/free-trial`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${accessToken}`,
  ...
  const currentPlan = currentSubscription?.data?.subscription?.productId;
          ...
          >
            ...
            {Number(plan?.price) <= 0 ? (
              <button
                ...
                disabled={
                  freePending ||
                  currentSubscription?.data?.subscription?.productId ===
                    "free_trial"
                }
                className={cn(
                  "w-full py-2.5 rounded-lg text-[13px] font-semibold transition-colors mb-6 disabled:opacity-50! disabled:cursor-not-allowed",
                  plan.isRecommended
              ...
              (() => {
                ...
                const isActive = plan.productId === activeProductId;
                ...
                return (
                  <a
                    href={
                      isActive
                        ? undefined
                        : `https://pay.rev.cat/sandbox/aaxsusjnyvmqcrjo/${profile?.id}`
                    }
                    onClick={(e) => {
                  ...
                  >
                    <Button
                      disabled={isActive}
                      className="w-full py-2.5 rounded-lg text-[13px] font-semibold transition-colors mb-6 disabled:opacity-50! disabled:cursor-not-allowed"
                    >
